...
ShellRoot {
    ...
    property string defaultWallpaperPath: wallpaperPath + "/appa.webp"
    ...
    function resolveScreen(monitorName, fallbackScreen) {
        monitorName = monitorName || ""
        if (monitorName !== "") {
            for (var i = 0; i < Quickshell.screens.length; i++) {
                var candidate = Quickshell.screens[i]
                var monitor = Hyprland.monitorFor(candidate)
                if (monitor && monitor.name === monitorName) return candidate
                if (!monitor && candidate.name === monitorName) return candidate
            }
        }

        return fallbackScreen || firstScreen()
    }

    function panelPlacement(currentScreen, monitorName) {
        var nextScreen = resolveScreen(monitorName, currentScreen)
        return {
            screen: nextScreen,
            moved: !sameScreen(currentScreen, nextScreen),
        }
    }

    function toggleLauncher(monitorName) {
        var placement = panelPlacement(launcherScreen, monitorName)
        launcherScreen = placement.screen
        launcherVisible = !launcherVisible || placement.moved
        return launcherVisible
    }

    function toggleDashboard(monitorName) {
        var placement = panelPlacement(dashboardScreen, monitorName)
        dashboardScreen = placement.screen
        dashboardVisible = !dashboardVisible || placement.moved
        if (dashboardVisible) { wifiVisible = false; btVisible = false }
        return dashboardVisible
    }

    function toggleMusic(monitorName) {
        var placement = panelPlacement(musicScreen, monitorName)
        musicScreen = placement.screen
        musicVisible = !musicVisible || placement.moved
        return musicVisible
    }

    function toggleWifi(monitorName) {
        var placement = panelPlacement(wifiScreen, monitorName)
        wifiScreen = placement.screen
        wifiVisible = !wifiVisible || placement.moved
        if (wifiVisible) { btVisible = false; dashboardVisible = false; refreshWifi() }
        return wifiVisible
    }

    function toggleBluetooth(monitorName) {
        var placement = panelPlacement(btScreen, monitorName)
        btScreen = placement.screen
        btVisible = !btVisible || placement.moved
    ...
    Process {
        id: currentWallProc
        command: ["bash", "-c",
            "current=$(readlink -f '" + root.wallpaperPath + "/current' 2>/dev/null || true) && " +
            "if [ -f '" + root.defaultWallpaperPath + "' ]; then " +
            "  echo '" + root.defaultWallpaperPath + "'; " +
        ...
        onExited: {
            if (root.currentWallpaper !== "") {
                root.applyWallpaper({
                    ...
                    name: root.currentWallpaper.split("/").pop()
                })
            }
        }
    }
    ...
    Dashboard { screen: root.dashboardScreen }
    MusicPanel { screen: root.musicScreen }
    WifiPanel { screen: root.wifiScreen }
    BluetoothPanel { screen: root.btScreen }
    LauncherPanel { screen: root.launcherScreen }

    IpcHandler {
        target: "launcher"
        function toggle(monitorName: string) {
            root.activeTab = 0
            root.toggleLauncher(monitorName || "")
        }
    }
    IpcHandler {
        target: "dashboard"
        function toggle(monitorName: string) { root.toggleDashboard(monitorName || "") }
    }
    IpcHandler {
        target: "music"
        function toggle(monitorName: string) { root.toggleMusic(monitorName || "") }
    }
    IpcHandler {
        target: "wallpaper"
        function toggle(monitorName: string) {
            var placement = root.panelPlacement(root.launcherScreen, monitorName || "")
            root.launcherScreen = placement.screen

            if (!root.launcherVisible || placement.moved) {
                root.activeTab = 1
    ...
    IpcHandler {
        target: "wifi"
        function toggle(monitorName: string) { root.toggleWifi(monitorName || "") }
    }
    IpcHandler {
        target: "bluetooth"
        function toggle(monitorName: string) { root.toggleBluetooth(monitorName || "") }
    }
}
