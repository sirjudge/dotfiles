...
PanelWindow {
    ...
    property string monitorName: hyprMonitor ? hyprMonitor.name : (screen ? screen.name : "")
    ...
    function currentWorkspaceId() {
        if (bar.hyprMonitor && bar.hyprMonitor.activeWorkspace) {
            return bar.hyprMonitor.activeWorkspace.id
        }
    ...
    function syncWorkspace(animated) {
        var wsId = currentWorkspaceId()
        if (wsId === bar.activeWsId && wsId === bar.targetWsId) return

        bar.targetWsId = wsId

        if (animated && bar.activeWsId !== wsId) {
    ...
    function workspaceBelongsToBarMonitor(workspace) {
        if (!workspace || workspace.id <= 0) return false
        if (!workspace.monitor) return true

        if (screen && workspace.monitor.name === screen.name) {
            return true
        }

        if (!bar.hyprMonitor) return true

        return workspace.monitor === bar.hyprMonitor
            || workspace.monitor.id === bar.hyprMonitor.id
            || workspace.monitor.name === bar.hyprMonitor.name
    }

    Connections {
        target: bar.hyprMonitor
        function onActiveWorkspaceChanged() {
            bar.syncWorkspace(true)
        }
    }
    ...
    Component.onCompleted: {
        bar.syncWorkspace(false)
    }

    onScreenChanged: hyprMonitor = screen ? Hyprland.monitorFor(screen) : null
    onHyprMonitorChanged: syncWorkspace(false)
    ...
    Item {
                ...
                MouseArea {
                    ...
                    onClicked: function(mouse) {
                        if (mouse.button === Qt.RightButton) {
                            var placement = root.panelPlacement(root.launcherScreen, bar.monitorName)
                            root.launcherScreen = placement.screen
                            root.activeTab = 1
                            if (!root.launcherVisible || placement.moved) root.launcherVisible = true
                            if (!root.wallsLoaded) root.loadWallpapers()
                        } else {
                            root.activeTab = 0
                            root.toggleLauncher(bar.monitorName)
                        }
                    }
                }
            }
                        ...
                        Row {
                            ...
                            Repeater {
                                ...
                                delegate: Item {
                                    ...
                                    property bool onBarMonitor: bar.workspaceBelongsToBarMonitor(modelData)
                                    property bool isActive: onBarMonitor && modelData.active
                                    property bool isHovered: wsMA.containsMouse
                                    property bool isVisibleWorkspace: onBarMonitor

                                    visible: isVisibleWorkspace
                                    width: isVisibleWorkspace ? Math.max(wsText.implicitWidth + 14, 26) : 0
                                    height: isVisibleWorkspace ? 18 : 0

                                    onIsActiveChanged: updateHighlight()
                                    onOnBarMonitorChanged: updateHighlight()
                                    ...
                                    function updateHighlight() {
                                        if (isActive && isVisibleWorkspace) {
        ...
        Notch {
            ...
            MouseArea {
                ...
                onClicked: function(mouse) {
                    if (mouse.button === Qt.RightButton)
                        root.toggleMusic(bar.monitorName)
        ...
        Row {
            ...
            Notch {
                ...
                MouseArea {
                    ...
                    onClicked: function(mouse) {
                        if (mouse.button === Qt.RightButton)
                            root.toggleBluetooth(bar.monitorName)
                        else
                            root.toggleWifi(bar.monitorName)
                    }
                }
            }
            ...
            Notch {
                ...
                MouseArea {
                    ...
                    hoverEnabled: true
                    cursorShape: Qt.PointingHandCursor
                    onClicked: root.toggleDashboard(bar.monitorName)
                }
            }
        }
    }
