return {
  ...
  config = function()
    ...
    vim.keymap.set({ "n", "x" }, "<leader>ogo",  function() return require("opencode").operator("@this ") end,        { desc = "Add range to opencode", expr = true })
    vim.keymap.set("n",          "<leader>ogoo", function() return require("opencode").operator("@this ") .. "_" end, { desc = "Add line to opencode", expr = true })

    -- note: These are default mappings, no idea if I want to keep them or not
